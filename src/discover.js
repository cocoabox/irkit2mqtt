...
function just_request(ip) {
    return new Promise(async (resolve) => {
        try {
            await axios.request(`http://${ip}/` , {method : 'get'});
        } catch (e) {
            // ignore any errors
        }
        resolve();
...
function discover(target , opts = {}) {
    ...
    return new Promise(async (resolve , reject) => {
        try {
            ...
            for ( const ip of ips ) {
                console.log('[discover] just fetching' , ip); // this updates the arp cache
                try {
                    await just_request(ip);
                } finally {
                    // we're just making a sample HTTP call so that we'll get the ARP record
                }
            }
            try {
