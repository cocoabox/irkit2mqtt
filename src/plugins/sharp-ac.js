...
class SharpA909JB extends IrkitAppliance {
    ...
    constructor({irkit_inst , setup , restore_state_cache , log} = {}) {
        super({
            irkit_inst ,
            setup : Object.assign({simple : true} , setup ?? {}) ,
            restore_state_cache ,
            log
        });
        this.log('[SharpA909JB] my state is:' , this.states);
        this.log('[SharpA909JB] setting initial state');
    ...
    after_set_state() {
        const current_mode = this.get_state('mode');
        if ( ['warm' , 'dry' , 'indoor_drying'].includes(current_mode) ) {
            if ( this.get_state('direction') === 'ceiling' ) {
                this.log.warn(`current mode "${current_mode}" does not support direction=ceiling; setting to auto`);
    ...
    create_irkit_data({
        ...
        // this.log("1..8  :", irdata.bits.join('').replaceAll(/(.{4})/g, '$1 ' ));
        ...
        // this.log("1..9  :", irdata.bits.join('').replaceAll(/(.{4})/g, '$1 ' ));
        ...
        // this.log("1..10 :", irdata.bits.join('').replaceAll(/(.{4})/g, '$1 ' ));

        // --- #11 : constant 1000
        irdata.append_nibble(new Nibble(0x08));
        // this.log("1..11 :", irdata.bits.join('').replaceAll(/(.{4})/g, '$1 ' ));
        ...
        // this.log("1..12 :", irdata.bits.join('').replaceAll(/(.{4})/g, '$1 ' ));
        ...
        switch (true) {
            case timer === 'on' && half_hour:
                irdata.append_nibble(new Nibble(0b1011));
                break;
            case timer === 'on':
            case timer === 'on_cancel' :
                irdata.append_nibble(new Nibble(0b0011));
                break;
            case timer === 'off' && half_hour:
                irdata.append_nibble(new Nibble(0b1000));
                break;
            case timer === 'off':
            case timer === 'off_cancel' :
                irdata.append_nibble(new Nibble(0b0000));
                break;
            default:
                irdata.append_nibble(new Nibble(0b0000));
        }
        ...
        this.log.info('generated infrared transmission for state' , {
            power , mode , temp , direction , strength , timer ,
            timer_hours , internal_clean , ion , power_saving , dry_hours , manually_assign
        } , '==>' , irdata.bits.join('').replace(/[]/g).replace(/(\d{4})/g , '$1 '));
    ...
    generate_irkit_data() {
        ...
        const simple = this.get_state('simple' , true);
        ...
        const multi =
            ['on' , 'off' , 'on_cancel' , 'off_cancel'].includes(timer)
            || ! power
            || simple
                ? [ // only simulate the MODE button
                    this.create_irkit_data(Object.assign({} , base)) ,
                ]
                : [ // simulate the WIND DIRECTION, TEMP, STRENGTH buttons separately ?
                    this.create_irkit_data(Object.assign({} , base , {manually_assign : 'mode'})) ,
                    {sleep : 500} ,
